...
<div class="max-w-5xl mx-auto p-6">
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-red-600">🇻🇪 Editar Usuario y Perfil</h1>
    ...
    <form method="POST" action="{{ route('admin.usuarios-perfiles.update', $usuarios_perfile) }}">
        @csrf @method('PUT')

        {{-- DATOS DEL USUARIO --}}
        <div class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-xl shadow-lg p-6 ring-2 ring-blue-200 dark:ring-gray-700 mb-6">
            <h2 class="text-xl font-bold text-blue-700 dark:text-blue-300 mb-4">Datos del Usuario</h2>
            <div class="grid md:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300">Nombre</label>
                    <input type="text" name="name" value="{{ old('name', $usuarios_perfile->name) }}" required class="w-full mt-1 rounded-lg border-blue-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 shadow">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300">Apellido</label>
                    <input type="text" name="apellido" value="{{ old('apellido', $usuarios_perfile->apellido) }}" required class="w-full mt-1 rounded-lg border-blue-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 shadow">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300">Cédula / DNI</label>
                    <input type="text" value="{{ $usuarios_perfile->dni }}" disabled class="w-full mt-1 rounded-lg border-gray-300 dark:border-gray-600 bg-gray-100 dark:bg-gray-700 text-gray-500">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300">Teléfono</label>
                    <input type="text" name="telefono" value="{{ old('telefono', $usuarios_perfile->telefono) }}" class="w-full mt-1 rounded-lg border-blue-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 shadow">
                </div>
                <div class="md:col-span-2">
                    <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300">Email</label>
                    <input type="email" name="email" value="{{ old('email', $usuarios_perfile->email) }}" required class="w-full mt-1 rounded-lg border-blue-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 shadow">
                </div>
            </div>
        </div>

        {{-- DATOS DEL PERFIL --}}
        <div class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-xl shadow-lg p-6 ring-2 ring-red-200 dark:ring-gray-700">
            <h2 class="text-xl font-bold text-red-700 dark:text-red-300 mb-4">Datos del Perfil</h2>
            <div class="grid md:grid-cols-2 gap-4">
                <div>
